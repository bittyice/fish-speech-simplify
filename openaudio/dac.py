...
@dataclass
class TransformerBlockConfig(AttentionConfig, FeedForwardConfig):
    pass
...
@dataclass
class WindowLimitedTransformerConfig(TransformerConfig):
    input_dim: int = 512
    window_size: int = 512

@dataclass
class EncoderBlockConfig(WindowLimitedTransformerConfig):
    stride: int = 1
...
class EncoderConfig():
    ...
    blocks: list[EncoderBlockConfig] = []
...
class DecoderConfig:
    ...
    blocks: list[DecoderBlockConfig] = []

class DACConfig:
    sample_rate: int = 44100
    encoder_stride: list[int]
    encoder_n_layer: list[int]
    # 和 encoder_stride 反着来
    decoder_stride: list[int]
    encoder: EncoderConfig
    decoder: DecoderConfig

    pre_module: WindowLimitedTransformerConfig
    post_module: WindowLimitedTransformerConfig

    @staticmethod
    def create(
        encoder_stride = [2,4,8,8],
        encoder_n_layer = [0,0,0,4],
        # 和 encoder_stride 反着来
        decoder_stride = [8,8,4,2]
    ):
        config = DACConfig()
        config.encoder_stride = encoder_stride
        config.encoder_n_layer = encoder_n_layer
        config.decoder_stride = decoder_stride

        encoder_config = EncoderConfig()
        d_model = encoder_config.d_model
        for stride, n_layer in zip(config.encoder_stride, config.encoder_n_layer):
            d_model *= 2
            encoder_config.blocks.append(EncoderBlockConfig(
                input_dim=d_model,
                stride=stride,
                n_layer=n_layer,
                n_head=d_model // 64,
        ...
        for i, stride in enumerate([8, 8, 4, 2]):
            input_dim = decoder_config.channels // 2**i
            output_dim = decoder_config.channels // 2 ** (i + 1)

            decoder_config.blocks.append(DecoderBlockConfig(
                input_dim=input_dim,
        ...
        win_transformer_confing = WindowLimitedTransformerConfig(
            block_size=4096,
            dim=1024,
        ...
        config.pre_module=win_transformer_confing
        config.post_module=win_transformer_confing
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # z = self.attention_norm(x)#-0.1245
        # a = self.attention(z, freqs_cis, mask, input_pos)#-13.5625
        # b = self.attention_layer_scale(a)#-4.8340e-02
        # h = x + b#-0.5508

        # c = self.feed_forward(self.ffn_norm(h))#-39.2500
        # d = self.ffn_layer_scale(c)#-0.7969
        # out = h + d
        # return out#-1.343
...
class WindowLimitedTransformer(Transformer):
    ...
    def __init__(
        self,
        config: WindowLimitedTransformerConfig,
        causal: bool = True,
        ...
        self.input_proj = (
            ...
            if config.input_dim != config.dim
            else nn.Identity()
        )
        self.output_proj = (
            nn.Linear(config.dim, config.input_dim)
            if config.input_dim != config.dim
            else nn.Identity()
        )
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        conv_class = CausalWNConv1d if causal else WNConv1d
        self.block = nn.Sequential(
            ResidualUnit(config.input_dim // 2, dilation=1, causal=causal),
            ResidualUnit(config.input_dim // 2, dilation=3, causal=causal),
            ResidualUnit(config.input_dim // 2, dilation=9, causal=causal),
            Snake1d(config.input_dim // 2),
            conv_class(
                config.input_dim // 2,
                config.input_dim,
                kernel_size=2 * config.stride,
                stride=config.stride,
                padding=math.ceil(config.stride / 2),
            ),
            (
                nn.Identity()
                if config.n_layer == 0
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.block = [conv_class(1, config.d_model, kernel_size=7, padding=3)]
        ...
        output_dim = config.blocks[-1].input_dim
        # Create last convolution
        self.block += [
            Snake1d(output_dim),
            conv_class(output_dim, config.d_latent, kernel_size=3, padding=1),
        ]
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.block = nn.Sequential(
            ...
            ResidualUnit(config.output_dim, dilation=1, causal=causal),
            ResidualUnit(config.output_dim, dilation=3, causal=causal),
            ResidualUnit(config.output_dim, dilation=9, causal=causal),
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        layers = [conv_class(config.input_channel, config.channels, kernel_size=7, padding=3)]
        ...
        output_dim = config.blocks[-1].output_dim
        # Add final conv layer
        layers += [
            Snake1d(output_dim),
            conv_class(output_dim, config.d_out, kernel_size=7, padding=3),
            nn.Tanh(),
        ]
...
class DAC(nn.Module):
    def __init__(
        ...
        self.hop_length = np.prod(config.encoder_stride)
        ...
        self.quantizer = DownsampleResidualVectorQuantize(
            pre_module=WindowLimitedTransformer(config.pre_module, batch_frist=False), 
            post_module=WindowLimitedTransformer(config.post_module, batch_frist=False))
